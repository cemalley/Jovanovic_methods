...
genes.intersection <- intersect(row.names(D7astro.raw), row.names(LA_D0_D7.raw))
genes.intersection #27131

genes.filter_PF <- fread('/Volumes/ncatssctl/NGS_related/marker_sets/35473_Gene_To_Filter_PF.csv')
genes.filter_PF #35473
genes.intersection_PF <- genes.intersection[genes.intersection %nin% genes.filter_PF$GeneSymbol]
genes.intersection_PF #17186 genes left
length(genes.intersection_PF) #[1] 17186
gtools::mixedsort(genes.intersection_PF) #17186
genes_filtered <- genes.intersection_PF #17186 left
length(genes_filtered)

D7astro.raw <- subset(D7astro.raw, row.names(D7astro.raw) %in% genes_filtered)
D7astro.raw
class(D7astro.raw)
LA_D0_D7.raw <- subset(LA_D0_D7.raw, row.names(LA_D0_D7.raw) %in% genes_filtered)
LA_D0_D7.raw
class(LA_D0_D7.raw)
...
sampletable <- data.table('condition'=c(rep('LAday0NPC',3), rep('LAday7NPC',3), rep('AstroDay7',3)), 'replicate'=c('LAday0NPC_1','LAday0NPC_2','LAday0NPC_3','LAday7NPC_1','LAday7NPC_2','LAday7NPC_3','AstroDay7_1','AstroDay7_2','AstroDay7_3'))
...
fwrite(res, '/Volumes/ncatssctl/NGS_related/BulkRNA/Common_analysis/Cross-experiment/D7astro_vs_LAday7NPC_DE_IncludingDay0NPC.csv')
